...
import { buildEmailVerificationHtml, buildPasswordResetEmailHtml } from './templates/otp-email.template';
...
export class MailService {
  ...
  async sendEmailVerificationOtp(to: string, otp: string): Promise<void> {
    ...
    const content = buildEmailVerificationHtml({ otp, logoUrl: logoUrl || undefined });
  ...
  async sendPasswordResetOtp(to: string, otp: string): Promise<void> {
    ...
    const logoUrl = this.configService.get<string>('MAIL_LOGO_URL');
    const content = buildPasswordResetEmailHtml({ otp, logoUrl: logoUrl || undefined });
    await this.sendEmail(to, subject, content);
  }
}
